...
public static class ActivationFunctions
{
    ...
    public static double ReLUFunction(double x) => Math.Max(0.0, x);

    /// <summary>Computes the step function: 1 if x > 0, else 0.</summary>
    public static double StepFunction(double x) => x > 0.0 ? 1.0 : 0.0;

    /// <summary>Returns x unchanged (identity/passthrough).</summary>
    public static double IdentityFunction(double x) => x;
}
