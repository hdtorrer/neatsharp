...
public class ParallelAsyncFunctionAdapterTests
{
    [Fact]
    public async Task EvaluatePopulationAsync_AllGenomes_ReceiveCorrectFitnessMatchingSequential()
    {
        ...
        Func<IGenome, CancellationToken, Task<double>> fitnessFunc =
            (g, _) => Task.FromResult(g.NodeCount * 10.0);
    ...
    public async Task EvaluatePopulationAsync_SomeGenomesThrow_RemainingGetCorrectScoresAndExceptionContainsAllFailures()
    {
        ...
        Func<IGenome, CancellationToken, Task<double>> fitnessFunc = (g, _) =>
        {
    ...
    public async Task EvaluatePopulationAsync_CancellationRequested_AlreadyCompletedScoresPreserved()
    {
        ...
        Func<IGenome, CancellationToken, Task<double>> fitnessFunc = async (g, ct) =>
        {
            ...
            // Slow down remaining evaluations so cancellation can take effect
    ...
    [Fact]
    public async Task EvaluatePopulationAsync_MaxDegreeOfParallelism2_LimitsConcurrentAsyncEvaluationsTo2()
    {
        ...
        Func<IGenome, CancellationToken, Task<double>> fitnessFunc = async (g, _) =>
        {
    ...
    public async Task EvaluatePopulationAsync_AllGenomesThrow_AggregatedErrorContainsAllFailuresAndDefaultFitnessAssigned()
    {
        ...
        Func<IGenome, CancellationToken, Task<double>> fitnessFunc =
            (g, _) => throw new InvalidOperationException($"Failed for node count {g.NodeCount}");
